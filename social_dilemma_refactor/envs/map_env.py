...
ORIENTATIONS = {"LEFT": [0, -1], "RIGHT": [0, 1],
                "UP": [-1, 0], "DOWN": [1, 0]}

DEFAULT_COLOURS = {
    b" ": np.array([0, 0, 0], dtype=np.uint8),  # Black background
    # Black background beyond map walls
    ...
    # Generic agents (any player)
    b"P": np.array([159, 67, 255], dtype=np.uint8),
    ...
    # light saturated cyan (turquoise)
    b"U": np.array([0, 255, 255], dtype=np.uint8),
}
...
class MapEnv(ParallelEnv):
    def __init__(
        ...
        self.world_map_color = np.full(
            (len(self.base_map) + view_len * 2,
             len(self.base_map[0]) + view_len * 2, 3),
            fill_value=0,
            dtype=np.uint8,
        )

        self.beam_pos = []
        # List:[agents objective]
        self.agents = []
        self.possible_agents = []
        # Dict: {agents id: agents objective}
        self.agents_dict = {}
        # returns the agents at a desired position if there is one
        self.pos_dict = {}
    ...
    def reset(self, seed=None, return_info=None, options=None):
        ...
        for agent in self.agents_dict.values():
            ...
            if self.return_agent_actions:
                ...
                prev_actions = np.array([4 for _ in range(self.num_of_agents - 1)]).astype(np.uint8)
    ...
    def step(self, actions):
        ...
        Parameters
        ----------
        actions: dict {agents-id: int}
            dict of actions, keyed by agents-id that are passed to the agents. The agents
            interprets the int and converts it to a command

        Returns
        -------
        observations: dict of arrays representing agents observations
        rewards: dict of rewards for each agents
        dones: dict indicating whether each agents is done
        ...
        for agent_id, action in actions.items():
            agent_action = self.agents_dict[agent_id].action_map(action)
            # print(agent_action)
        ...
        for agent in self.agents_dict.values():
            ...
            # concatenate on the prev_actions to the observations
            if self.return_agent_actions:
                prev_actions = np.array([actions[key] for key in sorted(actions.keys()) if key != agent.agent_id]).astype(np.uint8)
                visible_agents = self.find_visible_agents(agent.agent_id)
                observations[agent.agent_id] = {
                    "curr_obs": rgb_arr,
                    "other_agent_actions": prev_actions,
        ...
        if self.inequity_averse_reward:
            ...
            for index, agent_id in enumerate(rewards.keys()):
                ...
                temp_rewards[agent_id] -= (dis_inequity + adv_inequity) / (self.num_of_agents - 1)
            rewards = temp_rewards

        dones["__all__"] = np.any(list(dones.values()))
        infos.update(self.get_custom_infos())
    ...
    def custom_action(self, agent, action):
        ...
        Parameters
        ----------
        agent: agents that is taking the action
    ...
    # FIXME(ev) move this to a env_utils eventually
    ...
    @property
    def agent_pos(self):
        return [agent.pos.tolist() for agent in self.agents_dict.values()]

    def get_map_with_agents(self):
        """Gets a version of the environment map where generic
        'P' characters have been replaced with specific agents IDs.
        ...
        for agent in self.agents_dict.values():
            ...
            # If agents is not within map, skip.
    ...
    def check_agent_map(self, agent_map):
        ...
        for i in range(self.num_of_agents):
            if count_dict[chr(i + 1)] != 1:
                print("Error! Wrong number of agents", i, "in map!")
                return False
        return True
    ...
    def map_to_colors(self, mmap, color_map, rgb_arr, orientation="UP"):
        ...
        if orientation == "UP":
            ...
            for row_elem in range(x_len):
                for col_elem in range(y_len):
                    rgb_arr[row_elem, col_elem, :] = color_map[mmap[x_len - 1 - row_elem, y_len - 1 - col_elem]]
    ...
    def update_moves(self, agent_actions):
        """Converts agents action tuples into a new map and new agents positions.
        ...
        conflict with another agents for a cell, but may not be temporarily resolvable
        the actions until all moves have been satisfied or deemed impossible.
        For example, agents 1 may want to move from [1,2] to [2,2] but agents 2 is in [2,2].
        Agent 2, however, is moving into [3,2]. Agent-1's action is first in the order so at the
        first pass it is skipped but agents-2 moves to [3,2]. In the second pass, agents-1 will
        ...
        # helpful for finding the agents in the conflicting slot
        agent_by_pos = {tuple(agent.pos): agent.agent_id for agent in self.agents_dict.values()}

        # agents moves keyed by ids
        agent_moves = {}
        ...
        if len(agent_to_slot) > 0:
            ...
            # shuffle so that a random agents has slot priority
            shuffle_list = list(zip(agent_to_slot, move_slots))
            np.random.shuffle(shuffle_list)
            agent_to_slot, move_slots = zip(*shuffle_list)
            unique_move, indices, return_count = np.unique(move_slots, return_index=True, return_counts=True, axis=0)
            search_list = np.array(move_slots)

            # first go through and remove moves that can't possible happen. Three types
            # 1. Trying to move into an agents that has been issued a stay command
            # 2. Trying to move into the spot of an agents that doesn't have a move
            ...
            if np.any(return_count > 1):
                for move, index, count in zip(unique_move, indices, return_count):
                    if count > 1:
                        ...
                        # contain an agents that isn't going to move for one of the agents
                        ...
                        for agent_id in all_agents_id:
                            ...
                            if move.tolist() in self.agent_pos:
                                # find the agents that is currently at that spot and make sure
                                # that the move is possible. If it won't be, remove it.
                                conflicting_agent_id = agent_by_pos[tuple(move)]
                                curr_pos = self.agents_dict[agent_id].pos.tolist()
                                curr_conflict_pos = self.agents_dict[conflicting_agent_id].pos.tolist()
                                ...
                                # or you are trying to move into an agents that hasn't
                                # received a command
                                elif (
                                ...
                                # It is trying to move into you and you are moving into it
                                    ...
                                    if (
                                            ...
                                            and move.tolist() == self.agents_dict[conflicting_agent_id].pos.tolist()
                                    ):
                                        conflict_cell_free = False
                        ...
                        if conflict_cell_free:
                            self.agents_dict[agent_to_slot[index]].update_agent_pos(move)
                            agent_by_pos = {
                                tuple(agent.pos): agent.agent_id for agent in self.agents_dict.values()
                            }
                        # ------------------------------------
                        ...
                        # to stay in place
                        for agent_id in all_agents_id:
                            agent_moves[agent_id] = self.agents_dict[agent_id].pos.tolist()
            ...
            while len(agent_moves.items()) > 0:
                ...
                for agent_id, move in moves_copy.items():
                    ...
                    if move in self.agent_pos:
                        ...
                        # Condition (2)
                        # its command is to stay
                        # or you are trying to move into an agents that hasn't received a command
                        elif (
                                conflicting_agent_id not in moves_copy.keys()
                                or curr_conflict_pos == conflict_move
                        ):
                        ...
                        # It is trying to move into you and you are moving into it
                            ...
                            if (
                                    agent_moves[conflicting_agent_id] == curr_pos
                                    and move == self.agents_dict[conflicting_agent_id].pos.tolist()
                            ):
                    ...
                    # this move is unconflicted so go ahead and move
                        ...
                        self.agents_dict[agent_id].update_agent_pos(move)
                        del agent_moves[agent_id]
                        del_keys.append(agent_id)

                # no agents is able to move freely, so just move them all
    ...
    def update_custom_moves(self, agent_actions):
        ...
        The order in which agents actions are resolved is random to ensure homogeneity, similar to
    ...
    def reset_map(self):
        ...
        self.world_map = np.full(
            (len(self.base_map), len(self.base_map[0])), b" ", dtype="c")
        self.world_map_color = np.full(
            (len(self.base_map) + self.view_len * 2,
             len(self.base_map[0]) + self.view_len * 2, 3),
            fill_value=0,
            dtype=np.uint8,
        )
        self.build_walls()
    ...
    def update_map_fire(
        ...
        Notes:
            ...
            (4) As per the rules, the beams fire from in front of the agents and on its
                sides so the beam that starts in front of the agents travels out one
                cell further than it does along the sides.
            (5) This method updates the beam_pos, an internal representation of how
                which cells need to be rendered with fire_char in the agents view
        ...
        """
        agent_by_pos = {tuple(agent.pos): agent_id for agent_id,
        agent in self.agents_dict.items()}
            ...
            for i in range(fire_len):
                if (
                    ...
                    if [next_cell[0], next_cell[1]] in self.agent_pos:
                        ...
                        self.agents_dict[agent_id].hit(fire_char)
                        break
    ...
    def spawn_point(self):
        ...
        spawn_index = 0
        is_free_cell = False
        curr_agent_pos = [agent.pos.tolist() for agent in self.agents_dict.values()]
    ...
    def spawn_rotation(self):
        """Return a randomly selected initial rotation for an agents"""
    ...
    # TODO(ev) this should be an agents property
    ...
    # TODO(ev) this definitely should go into env_utils or the general agents class
    ...
    def find_visible_agents(self, agent_id):
        """Returns all the agents that can be seen by agents with agent_id
        Args
        ----
        agent_id: str
            The id of the agents whose visible agents we are asking about
        Returns
        -------
        visible_agents: list
            which agents can be seen by the agents with id "agent_id"
        """
        agent_pos = self.agents_dict[agent_id].pos
        upper_lim = int(agent_pos[0] + self.agents_dict[agent_id].row_size)
        lower_lim = int(agent_pos[0] - self.agents_dict[agent_id].row_size)
        left_lim = int(agent_pos[1] - self.agents_dict[agent_id].col_size)
        right_lim = int(agent_pos[1] + self.agents_dict[agent_id].col_size)

        # keep this sorted so the visibility matrix is always in order
        other_agent_pos = [
            self.agents_dict[other_agent_id].pos
            for other_agent_id in sorted(self.agents_dict.keys())
